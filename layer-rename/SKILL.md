...
For each selected node, include the **selected node itself** in the rename plan (not just its children). Then call `get_nodes_info` to get its children recursively until the full tree is read.
