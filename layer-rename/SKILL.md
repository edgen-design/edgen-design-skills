...
For each selected node, call `get_nodes_info` to get its children. If children also have children, call `get_nodes_info` again recursively until the full tree is read.
