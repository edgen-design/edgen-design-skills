...
**Always use `scan_nodes_by_types` — never use `get_node_info` or `get_nodes_info` to read a full frame tree.** Those tools return the entire subtree (100KB+) and waste tokens.

For each selected frame:
1. Call `scan_nodes_by_types(nodeId, ["FRAME"])` — returns a flat list of all FRAME nodes with id, name, type, bbox
2. Filter for nodes whose name matches `Frame \d+`, `Group \d+`, `Rectangle \d+`, or other generic Figma auto-names
3. Use bbox position and neighbouring node names (from the same flat list) to infer context and generate semantic names — no further API calls needed in most cases
4. If a node's context is genuinely ambiguous (both itself and its neighbours are unnamed), call `get_node_info` on **that specific node only** — not the whole frame

**Never call `get_node_info` on a top-level frame.** Only call it on individual ambiguous nodes when the flat list alone is insufficient.
...
- **Component Instances** (type = `INSTANCE`, IDs starting with `I`) → SKIP, never rename instances
